...
function begin(done) {
  queryFromPoolWithConnection('BEGIN', (err, results, connection) => {
    done(err, connection);
  });
}
...
function query(connection, opts, values, done) {
  if (typeof values === 'function') {
    done = values;
    values = [];
  }
  ...
  if (!connection) {
    queryFromPool(opts, values, done);
  } else {
    connection.query(opts, values, done);
  }
}

function lockedFunction(lock_name, func, done) {
  ...
  async.series(
    ...
    (err) => {
      if (connection) {
        ...
        query(connection, sql, lock_name, () => {
          release(connection);
          done && done(err);
        });
      }
